...
class Dice
{
    ...
    public function create($component, array $args = [], $forceNewInstance = false)
    {
        ...
        if (empty($this->cache[$component])) {
            ...
            $this->cache[$component] = function ($args, $forceNewInstance) use ($component, $rule, $class, $constructor, $params) {
                ...
                if ($rule->call) foreach ($rule->call as $call) {
                    $class->getMethod($call[0])
                        ->invokeArgs($object, call_user_func($this->getParams($class->getMethod($call[0]), new Rule), $this->expand($call[1])));
                }
                return $object;
            };
        }
    ...
    {
        if (is_array($param)) {
            return array_map(function ($p) use ($share) {
                return $this->expand($p, $share);
            }, $param);
        }
        if ($param instanceof Instance && is_callable($param->name)) {
    ...
    private function getParams(\ReflectionMethod $method, Rule $rule)
    {
        ...
        return function ($args) use ($paramClasses, $rule, $subs) {
            $share = $rule->shareInstances ? array_map([
                    $this,
                    'create'
                ], $rule->shareInstances) : [];
            ...
            foreach ($paramClasses as $class) {
                ...
                if ($subs && isset($subs[$class])) {
                    $parameters[] = is_string($subs[$class]) ? $this->create($subs[$class]) : $this->expand($subs[$class]);
                } else if ($class) {
                    $parameters[] = $this->create($class, $share, $rule->newInstances && in_array(strtolower($class), array_map('strtolower', $rule->newInstances)));
                } else if ($args) {
